...
本文介绍 SPIRE 如何向工作负载颁发身份的详细步骤，从代理在节点上启动到同一节点上的工作负载收到 X.509 SVID 形式的有效身份。请注意，JWT 格式的 SVID 的处理方式是不同的。为了简单演示的目的，工作负载在 AWS EC2 上运行。
...
16. 现在完全引导，代理开始监听 Workload API 套接字。
17. 工作负载调用工作负载 API 来请求 SVID。
18. 代理通过调用其工作负载证明者来启动工作负载证明过程，并向他们提供工作负载进程的进程 ID。
19. 证明者使用内核和用户空间调用来发现有关工作负载的其他信息。
20. 证明者以工作负载选择器的形式将发现的信息返回给代理。
...
2. 在数据库中查询特定代理与哪些附加属性相关联（“*节点*选择器”）。
3. 查询数据库以获取在任何这些节点*选择器上声明至少一个选择的任何注册条目。
