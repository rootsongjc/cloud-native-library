...
With an EastWest gateway, any internal service can be made highly available with automated cross-cluster failover without publishing it for external access via an Ingress gateway.

In this guide, you'll:

- Deploy [bookinfo application](https://istio.io/latest/docs/examples/bookinfo/) in one cluster `cluster-1`.  You'll deploy the same bookinfo application in a second cluster `cluster-2`.
...
When services are added to an EastWest gateway, Tetrate Service Bridge maintains an internal registry of these services. EastWest gateways are associated with a workspace, and by default, all services are registered with the gateway.
